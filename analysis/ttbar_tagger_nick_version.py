...
parser.add_option('--decayType', metavar='F', type='string', action='store',default='none',dest='decayType',help='') 
							...
							## 'WQQ', 'Wc', or 'Wlight'
							## use only for ttbar samples or W+Jets samples
...
parser.add_option('--sample_type', metavar='F', type='string', action='store',default='signal',dest='sample_type',help='') #Type of sample, needed to apply btagging efficiency
																															#Also identifites mcatnlo_signal
...
parser.add_option('--PdfVersion', metavar='F', type='string', action='store',default='CT10',dest='PdfVersion',help='') # Options for different PDF set, only meaningful for signal MC
...
t.Branch('weight_gen_event',    weight_gen_event,    'weight_gen_event/F')
...
f1 = TFile('/uscms_data/d3/eminizer/CMSSW_5_3_11/src/Analysis/EDSHyFT/test/tagging/dumped_Powheg_TT.root')
bdisc = f1.Get('bDisc')
...
if 'signal' in options.sample_type or options.sample_type == 'Powheg_dilep_TT' or options.sample_type == 'Powheg_had_TT' or options.sample_type == 'Powheg_qq_semilep_TT' or options.sample_type == 'Powheg_gg_semilep_TT' :
...
for event in events:
	...
	if options.mc == 'yes' :
		...
		if GenHandle.isValid() :
			...
			for ig in GenParticles :
				...
				#mcatnlo algorithm
				if options.sample_type == 'mcatnlo_signal' and math.fabs(ig.pdgId())!=6 and ig.status()==3 :
					...
					if ntopdaus == 2 :
						original_particles.append(ig.pdgId())
				if options.sample_type == 'WJets' :
					#Look for extra heavy flavor jets
					if math.fabs(ig.pdgId()) == 4 :
			...
			if (options.decayType == 'had' or options.decayType == 'hadronic') and not len(lep_ids) == 0 :
				continue
			#W+Jets event type cuts
			## 'WQQ', 'Wc', or 'Wlight'
			if options.decayType == 'WQQ' and (extra_bjets != 2 and extra_cjets != 2) :
				continue
			if options.decayType == 'Wc' and (extra_bjets == 2 or extra_cjets != 1) :
				continue
			if options.decayType == 'Wlight' and (extra_bjets > 0 or extra_cjets > 0) :
				continue 
	...
	#except the gen event weight
	weight_gen_event[0]		= 1.0
	if options.mc == 'yes' :
	...
	for i in range(len(leptonPts)):
		...
		if options.lep == 'mu' :
			...
			# if (leptonPts[i] < 26.0 or math.fabs(leptonEtas[i]) > 2.0) :
				continue
	...
	for i in range(len(dummypts)) :
		...
		# if toomanyjetPts[j] > 20. and math.fabs(toomanyjetEtas[j]) < 2.0 :
	...
	if (jetCands[0].Pt()<45.0 or jetCands[1].Pt()<35.0 or jetCands[2].Pt()<20.0 or jetCands[3].Pt()<20.0) :
		...
		endEvent(CUTFLOW)
		continue
	#cut on pts of hardest 4 jets (cuts from W helicity paper)
	#if (jetCands[0].Pt()<55.0 or jetCands[1].Pt()<45.0 or jetCands[2].Pt()<30.0 or jetCands[3].Pt()<20.0) :
	#	CUTFLOW = 3
	#	endEvent(CUTFLOW)
	#	continue
	count_hardEnough += 1
	...
	############################################################################################################
	#print "TLorentzVector* lepton = new TLorentzVector("+str(lepton.Px())+","+str(lepton.Py())+","+str(lepton.Pz())+","+str(lepton.T())+");"
	#print 'lepton mass = '+str(lepton.M())+''
	#print "TLorentzVector* met = new TLorentzVector("+str(met1.Px())+","+str(met1.Py())+","+str(met1.Pz())+","+str(met1.T())+");"
	#for jet in jetCands :
	#	print "TLorentzVector* jet = new TLorentzVector("+str(jet.Px())+","+str(jet.Py())+","+str(jet.Pz())+","+str(jet.T())+"); //pt = "+str(jet.Pt())
	#for jetCSV in jetCandCSVs :
	#	print "double jetCSV = "+str(jetCSV)+";"
	#print ''
	CUTFLOW = 5
	...
	if options.mc == 'yes' :
		...
		if GenHandle.isValid() :
			...
			for ig in GenParticles :
				...
				if ig.pdgId() == 2212 :
					factor = 0.0
					if ig.daughter(0).eta() > 0 :
						factor = 1.0
					elif ig.daughter(0).eta() < 0 :
						factor = -1.0
					if ig.daughter(0).pdgId() > 0 :
						mother1 = ROOT.TLorentzVector(1.0,0.0,factor*sqrt(BEAM_ENERGY*BEAM_ENERGY -1*1),BEAM_ENERGY)
					elif ig.daughter(0).pdgId() < 0 :
						mother2 = ROOT.TLorentzVector(1.0,0.0,factor*sqrt(BEAM_ENERGY*BEAM_ENERGY -1*1),BEAM_ENERGY)
				#Get the mother particles for mcatnlo events
				if options.sample_type == 'mcatnlo_signal' and math.fabs(ig.pdgId())!=6 and ig.status()==3 :
					#check if its daughters include the ttbar pair
					ntopdaus = 0
					for i in range(ig.numberOfDaughters()) :
	...
	if options.mc == 'yes' :
		#PDF REWEIGHTING
		if options.tMotherCheck == 'qq' or options.tMotherCheck == 'gg':
			#Get PDF weights
		...
		#top pT reweighting
		if 'signal' in options.sample_type or options.sample_type == 'Powheg_qq_semilep_TT' or options.sample_type == 'Powheg_gg_semilep_TT' :
			weight_top_pT[0] = math.sqrt(math.exp(0.159+(-0.00141)*mc_particles[6].Pt())*math.exp(0.159+(-0.00141)*mc_particles[7].Pt()))
		elif options.sample_type == 'Powheg_dilep_TT' :
	...
	weight_for_histos[0] = weight_gen_event[0]*weight_pileup[0]*weight_btag_eff[0]*weight_top_pT[0]*weight_tracking[0]*weight_lep_ID[0]*weight_lep_iso[0]*weight_trig_eff[0]
	if options.sideband == 'yes' :
		weight_for_histos[0] = weight_gen_event[0]*weight_pileup[0]*weight_btag_eff[0]*weight_top_pT[0]*weight_tracking[0]*weight_lep_ID[0]*weight_trig_eff[0]
